...
export function ResourceListItem({ resource, searchQuery = '', onClick, activeSkillFilters = [], activeSkillLevels = ['senior', 'mid', 'junior'] }: ResourceListItemProps) {
  ...
  return (
    <div 
      className="flex items-center gap-4 p-3 bg-card border border-border/50 rounded-lg shadow-sm dark:shadow-none hover:border-primary/20 hover:bg-accent/50 hover:shadow-lg hover:-translate-y-0.5 dark:hover:shadow-none transition-all duration-200 cursor-pointer"
      onClick={onClick}
    >
